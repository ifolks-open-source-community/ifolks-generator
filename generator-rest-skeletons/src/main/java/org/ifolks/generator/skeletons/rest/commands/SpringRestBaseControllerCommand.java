package org.ifolks.generator.skeletons.rest.commands;
...
public class SpringRestBaseControllerCommand extends JavaFileWriteCommand {
	...
	private void writePreAuthorize(String role) {
		if (role != null && !role.trim().isEmpty()) {
			writeLine("@PreAuthorize(\"hasAuthority('" + role.trim() + "')\")");
		} else {
			writeLine("@PreAuthorize(\"isAuthenticated()\")");
		}
	}
	...
	private void createGetOptions() {
		...
		if (bean.selectionBehavior.selectionMode.equals(SelectionMode.DROPDOWN_OPTIONS)) {			
			...
			writePreAuthorize(bean.accessRoles != null ? bean.accessRoles.readRole : null);
		...
		if (bean.selectionBehavior.selectionMode.equals(SelectionMode.AUTO_COMPLETE)) {			
			...
			writeLine(" */");
			writeLine("@RequestMapping(value = {" + bean.serviceInterfaceName + ".SEARCH_OPTIONS_URL }, method = RequestMethod.GET)");
			writePreAuthorize(bean.accessRoles != null ? bean.accessRoles.readRole : null);
			writeLine("public @ResponseBody List<SelectItem> searchOptions(@RequestParam(\"arg\") String arg) {");
	...
	private void createLoadObjectList() {
		...
		writePreAuthorize(bean.accessRoles != null ? bean.accessRoles.readRole : null);
		writeLine("public @ResponseBody List<" + this.bean.basicViewBean.recordName + "> loadList() {");
		...
		for (Property property : this.bean.properties) {
			if (property.referenceBean != null && property.relation.equals(RelationType.MANY_TO_ONE)) {
				...
				writeLine(" */");
				writeLine("@RequestMapping(value = {" + bean.serviceInterfaceName + ".GET_LIST_FROM_" + property.referenceBean.table.originalName + "_URL}, method = RequestMethod.GET)");
				writePreAuthorize(bean.accessRoles != null ? bean.accessRoles.readRole : null);
				writeLine("public @ResponseBody List<" + this.bean.basicViewBean.recordName + "> loadListFrom" + property.capName + " (@PathVariable(\"" + property.name + "Id\") " + property.referenceBean.idType + " " + property.name + "Id) {");
	...
	private void createScroll() {
		...
		writePreAuthorize(bean.accessRoles != null ? bean.accessRoles.readRole : null);
		writeLine("public @ResponseBody ScrollView<" + this.bean.basicViewBean.recordName + "> scroll(@RequestBody ScrollForm<" + bean.basicViewBean.filter.className + ", " + bean.basicViewBean.sortingClassName + "> form) {");
		...
		for (Property property : this.bean.properties) {
			if (property.referenceBean != null && property.relation.equals(RelationType.MANY_TO_ONE)) {
				...
				writeLine(" */");
				writeLine("@RequestMapping(value = {" + bean.serviceInterfaceName + ".SCROLL_FROM_" + property.referenceBean.table.originalName + "_URL}, method = RequestMethod.POST)");
				writePreAuthorize(bean.accessRoles != null ? bean.accessRoles.readRole : null);
				writeLine("public @ResponseBody ScrollView<" + this.bean.basicViewBean.recordName + "> scrollFrom" + property.capName + " (@PathVariable(\"" + property.name + "Id\") " + property.referenceBean.idType + " " + property.name + "Id, @RequestBody ScrollForm<" + bean.basicViewBean.filter.className + ", " + bean.basicViewBean.sortingClassName + "> form) {");
	...
	private void createLoadOneToManyComponentList() {
		for (OneToManyComponent oneToManyComponent : this.bean.oneToManyComponentList) {
			...
			writePreAuthorize(bean.accessRoles != null ? bean.accessRoles.readRole : null);
			writeLine("public @ResponseBody List<" + currentBean.basicViewBean.recordName + "> load" + currentBean.className + "List(@PathVariable(\"id\") " + currentBean.idType + " id) {");
	...
	private void createScrollOneToManyComponent() {
		for (OneToManyComponent oneToManyComponent : this.bean.oneToManyComponentList) {
			...
			writePreAuthorize(bean.accessRoles != null ? bean.accessRoles.readRole : null);
			writeLine("public @ResponseBody ScrollView<" + currentBean.basicViewBean.recordName + "> scroll" + currentBean.className + " (@PathVariable(\"id\") " + bean.idType + " id, @RequestBody ScrollForm<" + currentBean.basicViewBean.filter.className + ", " + currentBean.basicViewBean.sortingClassName + "> form) {");
	...
	private void createSaveObject() {
		...
		writePreAuthorize(bean.accessRoles != null ? (bean.accessRoles.createRole != null ? bean.accessRoles.createRole : bean.accessRoles.saveRole) : null);
	...
	private void createUpdateObject() {
		...
		writePreAuthorize(bean.accessRoles != null ? bean.accessRoles.updateRole : null);
	...
	private void createDeleteObject() {
		...
		writePreAuthorize(bean.accessRoles != null ? bean.accessRoles.deleteRole : null);
